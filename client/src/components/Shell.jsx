import { Bot, CheckSquare, Home, NotebookText, UserRound } from "lucide-react";
...
export function Shell({ children, current, go }) {
  const { subjects, user, isGuest } = useStudyBuddy();
  const topics = subjects.flatMap((subject) => subject.topics || []);

  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,#dbeafe,transparent_30%),linear-gradient(135deg,#f7f9ff,#f6f3ff)] pb-24">
      <header className="sticky top-0 z-20 border-b border-white/70 bg-white/78 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center gap-4 px-4 py-3">
          <button onClick={() => go("home")} className="flex items-center gap-2 rounded-lg text-left">
            ...
            <div>
              <p className="text-sm font-black leading-tight">StudyBuddy.AI</p>
              <p className="text-xs text-slate-500">{isGuest ? "Guest progress" : user?.name}</p>
            </div>
          </button>
          <div className="ml-auto hidden flex-1 max-w-md items-center rounded-xl bg-slate-100 px-4 py-2 text-sm text-slate-500 sm:flex">
            Search subjects, topics, notes...
          </div>
          <button
            onClick={() => go("profile")}
            className="grid h-10 w-10 place-items-center rounded-xl bg-white text-slate-700 shadow-sm"
            title="Profile"
          >
            <UserRound size={19} />
          </button>
      ...
      <footer className="fixed bottom-3 left-1/2 z-30 w-[min(94vw,620px)] -translate-x-1/2 rounded-2xl border border-white/80 bg-white/90 p-2 shadow-soft backdrop-blur-xl">
